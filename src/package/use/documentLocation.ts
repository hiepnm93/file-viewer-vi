...
const shouldSkipElement = (element: Element) => {
  return !!element.closest(DEFAULT_EXCLUDE_SELECTOR)
}
...
export const collectDocumentAnchors = (root: HTMLElement | null): FileViewerDocumentAnchor[] => {
  if (!root) {
    return []
  }
  ...
  const candidates = Array.from(root.querySelectorAll<HTMLElement>(DEFAULT_ANCHOR_SELECTOR))
...
export const findAnchorForElement = (
...
export const getCurrentDocumentAnchor = (
  root: HTMLElement | null,
  anchors: FileViewerDocumentAnchor[]
) => {
...
export const scrollToDocumentAnchor = (
  root: HTMLElement | null,
