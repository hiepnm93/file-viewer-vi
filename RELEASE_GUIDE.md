...
- `BRANCHES.md` / `WRAPPER_ECOSYSTEM.md`: 公开架构边界和生态仓库矩阵
...
状态报告会同时输出 `sourceBaseline`、`gaps`、`gapSummary` 和 `gapDetails`。`sourceBaseline` 明确私有 Gitea `main` 是完整原始聚合仓发布基线，并把本地 checkout 分支名只作为执行环境记录；`gapSummary` 用于快速区分本地可修问题和 npm/Gitee/GitHub 等外部发布阻塞，`gapDetails` 会为每条缺口标记 channel、scope、externalBlocker 和下一步动作。
