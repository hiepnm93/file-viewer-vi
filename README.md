...
只安装当前业务确实需要的 renderer。例如只预览 PDF：插件会按 `formats` 生成精确 import，并默认注入到页面，组件无需手动接收 `renderers`。
...
export const viewerOptions = {
  // 默认 true；仅在你要完全手动传 renderers 时设为 false。
  autoRenderers: true
}
```
...
重度用户、内部附件中心或演示平台需要一次拥有完整能力时，直接换成全量 preset，Vite 配置保持不变：

```bash
npm i @file-viewer/vue3 @file-viewer/core @file-viewer/vite-plugin @file-viewer/preset-all
```

如果希望在自动发现已安装 preset 的同时扫描源码 hint，使用 `preset:'auto'` 或 `autoPresets:true`：
