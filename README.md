...
当前版本以 npm registry 的 `latest` dist-tag 为准，共维护 27 个 npm 发布目标: 22 个标准组件/核心/renderer/preset 包 + 5 个历史兼容 alias。新项目建议优先使用 `@file-viewer/*` 标准包名；旧项目继续使用 `@flyfish-group/*` 或 `file-viewer3` 时也会拿到同版本能力。
