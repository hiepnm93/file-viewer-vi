...
- 按需装配可通过 `options.builtinRenderers` 控制内置集合: `all` 保持历史完整能力，`lite` 只启用图片、音视频和 UMD 等 core 原生轻量链路，`none` 适合完全由 `renderers` / preset 显式装配；代码、文本和 Markdown 请通过 `@file-viewer/renderer-text` 或 `@file-viewer/preset-all` 装配；当前默认仍是 `all`，后续完成重链路拆包后会切到更轻的默认体验
