...
- **格式覆盖完整。** 当前内置 198 个扩展名映射，覆盖 Word、Excel、PowerPoint、PDF、OFD、Typst、XMind 脑图、压缩包、邮件、OLB/DRA/GDS/OASIS、CAD、地理数据、3D 模型、Excalidraw、draw.io、EPUB、UMD、Markdown、图片、音频、视频、代码/文本、字体、设计资产和结构化数据，能覆盖绝大多数业务附件场景。
...
| EDA            | `olb`、`dra`、`gds`、`oas`、`oasis`                                                                                                                                                                                                                                                                                            | `@file-viewer/renderer-eda` 独立承接；使用 `cfb` 解析 OrCAD/Allegro 常见 CFB 容器；标准 GDSII 会读取 structure、boundary、path、text、reference 并生成 SVG 版图预览；OAS/OASIS 先做安全结构索引、可读字符串、实体候选和诊断，不在浏览器内执行专业电气校核 | 元件库、封装图纸、芯片版图附件初筛          |
...
| XMind 脑图     | `xmind`                                                                                                                                                                                                                                                                                                                        | 基于 `@ljheee/xmind-parser` 解析 XMind 8 XML 与 XMind 2020+ JSON 包结构，离线渲染多 sheet 脑图、节点、标签、备注、链接、标记、图片和目录树，支持 Pointer / 鼠标 / 触摸拖拽平移、移动端双指缩放、键盘平移、统一 toolbar 状态同步、WebView 异常 buttons 和混合事件兼容、适配画布、搜索、打印、HTML 导出和缩放 | 脑图、项目规划、知识结构、会议纪要          |
...
| 字体/设计/数据 | `ttf`、`otf`、`woff`、`woff2`、`psd`、`ai`、`eps`、`sqlite`、`wasm`、`parquet`、`avro`、`webarchive`                                                                                                                                                                                                                           | `@file-viewer/renderer-data` 独立承接，基于 FontFace、`ag-psd`、`sql.js`、`hyparquet`、`avsc`、WebAssembly Module 和安全摘要；SQLite WASM 支持私有化配置                                                           | 字体、设计资产、数据库、列式数据和 Web 归档 |
...
当前版本以 npm registry 的 `latest` dist-tag 为准，共维护 35 个 npm 发布目标: 30 个标准组件/核心/renderer/preset/工程插件包 + 5 个历史兼容 alias。新项目建议优先使用 `@file-viewer/*` 标准包名；旧项目继续使用 `@flyfish-group/*` 或 `file-viewer3` 时也会拿到同版本能力。
...
| 绘图 renderer                       | [`@file-viewer/renderer-drawing`](https://www.npmjs.com/package/@file-viewer/renderer-drawing)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                | 无                                                                                                                                                       | `latest` | 标准 renderer 插件，提供 Draw.io / diagrams.net 离线 viewer、Excalidraw 官方 SVG 导出、rough.js 兜底、缩放、打印和 HTML 导出                                            |
...
共享格式矩阵当前声明 24 条预览链路、198 个扩展名。完整能力通过 renderer / preset 按需装配，格式说明见本文“支持格式”和官方文档: https://doc.file-viewer.app/guide/formats
...
- 按需装配可通过 `options.builtinRenderers` 控制内置集合: `all` 保持历史完整能力，`lite` 只启用图片和 UMD 等 core 原生轻量链路，`none` 适合完全由 `renderers` / preset 显式装配；音视频请通过 `@file-viewer/renderer-media` 装配，代码、文本和 Markdown 请通过 `@file-viewer/renderer-text` 装配，也可以统一使用 `@file-viewer/preset-all`；当前默认仍是 `all`，后续完成重链路拆包后会切到更轻的默认体验
