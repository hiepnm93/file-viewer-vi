...
| XMind 脑图     | `xmind`                                                                                                                                                                                                                                                                                                                        | 基于 `@ljheee/xmind-parser` 解析 XMind 8 XML 与 XMind 2020+ JSON 包结构，离线渲染多 sheet 脑图、节点、标签、备注、链接、标记、图片和目录树，支持 Pointer / 鼠标 / 触摸拖拽平移、适配画布、搜索、打印、HTML 导出和缩放 | 脑图、项目规划、知识结构、会议纪要          |
...
当前版本以 npm registry 的 `latest` dist-tag 为准，共维护 34 个 npm 发布目标: 29 个标准组件/核心/renderer/preset/工程插件包 + 5 个历史兼容 alias。新项目建议优先使用 `@file-viewer/*` 标准包名；旧项目继续使用 `@flyfish-group/*` 或 `file-viewer3` 时也会拿到同版本能力。
...
| 全量 renderer preset                | [`@file-viewer/preset-all`](https://www.npmjs.com/package/@file-viewer/preset-all)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | 无                                                                                                                                                       | `latest` | 一次装配 PDF、OFD、PPTX、CAD、Draw.io/Excalidraw、Typst、XMind、压缩包、邮件、电子书、代码/Markdown、图片、音视频和 core 其余完整格式能力                              |
...
共享 core 当前声明 24 条预览链路、198 个扩展名。完整格式说明见本文“支持格式”和官方文档: https://doc.file-viewer.app/guide/formats
...
- 按需装配可通过 `options.builtinRenderers` 控制内置集合: `all` 保持历史完整能力，`lite` 只启用图片、音视频、代码、Markdown 和 UMD 等低成本链路，`none` 适合完全由 `renderers` / preset 显式装配；当前默认仍是 `all`，后续完成重链路拆包后会切到更轻的默认体验
