...
const sourceDir = resolve(repoDir, 'packages/compat/web/viewer')
const targetDirs = [
  resolve(demoDir, 'public/file-viewer'),
  resolve(demoDir, 'public/vendor/file-viewer')
]
const helperSourceDir = resolve(repoDir, 'packages/compat/web/dist')
...
if (!existsSync(resolve(sourceDir, 'flyfish-viewer-assets.json'))) {
  throw new Error('缺少 packages/compat/web/viewer/flyfish-viewer-assets.json，请先运行 pnpm build:viewer-assets')
}
