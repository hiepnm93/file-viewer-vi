...
`archive.zip` 与 `archive.tar.gz` 用于验证 `libarchive.js` 的 Worker 目录读取、按需解压、IndexedDB 缓存和压缩包内文件继续预览。`sample.eml`、`sample.msg` 与 `sample.mbox` 用于验证邮件头信息、HTML/文本正文、附件下载和归档识别。`sample.olb` 与 `sample.dra` 是 CFB 容器夹具，用于验证 OLB / DRA 的结构树、对象候选、属性、文本片段和可读字符串预览。`map.geojson`、`route.kml` 和 `track.gpx` 用于验证地理数据离线 SVG 预览；`sample.sqlite` 和 `module.wasm` 用于验证结构化数据与二进制摘要预览。
