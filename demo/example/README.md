...
CAD 预览使用 `@flyfish-dev/cad-viewer`。`sample.dwg` 已换成真实公开 DWG 文件，运行时会按需加载 viewer 静态目录下 `wasm/cad/` 中的 DWG Worker 和 LibreDWG WASM；`drawing.dxf` 继续验证 DXF parser、图层和视图适配；`samples/apache/blocks_and_tables.dwf` 来自 Apache Tika 公开 Jira 附件，用于验证 DWF 容器、块和表格渲染；`samples/autodesk/house.dwfx` 与 `samples/autodesk/robot-arm.dwfx` 来自 Autodesk 官方 Viewer 教程，用于验证 DWFx/XPS native renderer、W2D/W3D 图形和 `dwfv-render.wasm` fallback。
