...
通过 `options` 配置即可。`options.watermark` 支持文字或图片水印，`options.toolbar` 可以控制下载原文件、打印完整渲染内容和导出渲染后 HTML。打印和 HTML 导出会基于当前浏览器渲染结果生成，不会修改原始文件；PDF 会在打印时逐页生成完整页面，避免只打印当前视口或被滚动容器裁切。
