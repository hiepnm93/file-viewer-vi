...
Vue3 项目安装 `@flyfish-group/file-viewer3@1.0.12`，使用 `createApp(App).use(FileViewer)` 注册。Vue2.7 项目安装 `@flyfish-group/file-viewer@1.0.12`，使用 `Vue.use(FileViewer)` 注册。

React 项目安装 `@flyfish-group/file-viewer-react@1.0.12`，纯 JS 或非框架页面安装 `@flyfish-group/file-viewer-web@1.0.12`。React 和纯 JS 包只做 iframe 集成，默认加载宿主项目里的 `/file-viewer/index.html`，预览能力来自 Vue3 基线构建产物。
...
- Vue3 包 `@flyfish-group/file-viewer3` 已把样式挂到安装器里，正常 `app.use(FileViewer)` 就会一起带上
- Vue2 包 `@flyfish-group/file-viewer` 仍需要显式引入 `@flyfish-group/file-viewer/dist/style.css`
- React / 纯 JS 包本身不直接渲染样式，它们通过 iframe 加载私有化 viewer 静态产物

如果页面里能看到内容但排版像“裸 DOM”，优先检查是不是少引了对应版本的 CSS 文件，或者静态目录没有把 `file-viewer/index.html` 和 `file-viewer/assets/*` 一起发布出来。
...
如果你的包管理器没有执行安装脚本，或者静态目录不是 `public/file-viewer`，请运行:
...
通过 `options` 配置即可。`options.watermark` 支持文字或图片水印，`options.toolbar` 可以控制下载原文件、打印完整渲染内容和导出渲染后 HTML。打印和 HTML 导出会基于当前浏览器渲染结果生成，不会修改原始文件；PDF 会通过专属导出适配器逐页生成完整页面，避免只打印当前视口、当前页或被滚动容器裁切。

## 为什么 PDF 打印不是当前屏幕截图

从 `1.0.12` 起，PDF 的打印和导出 HTML 不再克隆当前可见 canvas，而是重新按页生成完整 PDF 页面。这样即使用户只滚动到第一页、开启了导航窗格，或当前视口里只渲染了部分页面，打印窗口和导出的 HTML 仍然会包含完整页序。
