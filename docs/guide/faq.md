...
Vue3 项目安装 `@flyfish-group/file-viewer3@1.0.11`，使用 `createApp(App).use(FileViewer)` 注册。Vue2.7 项目安装 `@flyfish-group/file-viewer@1.0.11`，使用 `Vue.use(FileViewer)` 注册。

React 项目安装 `@flyfish-group/file-viewer-react@1.0.11`，纯 JS 或非框架页面安装 `@flyfish-group/file-viewer-web@1.0.11`。React 和纯 JS 包只做 iframe 集成，默认加载宿主项目里的 `/file-viewer/index.html`，预览能力来自 Vue3 基线构建产物。
...
`.olb` / `.dra` 使用 `cfb` 解析常见 OrCAD / Allegro 复合文档容器，展示内部条目、文本片段和可读字符串。它适合做附件初筛，不替代专业 EDA 软件里的封装编辑、电气规则校核或制造输出。

## 水印、打印和导出怎么配置

通过 `options` 配置即可。`options.watermark` 支持文字或图片水印，`options.toolbar` 可以控制下载原文件、打印当前渲染内容和导出渲染后 HTML。打印和 HTML 导出会基于当前浏览器渲染结果生成，不会修改原始文件。
