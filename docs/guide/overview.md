...
- **格式策略务实。** PPTX 重点增强组合图形、主题背景、图片裁剪和 EMF 矢量图片；OFD 基于 `DLTech21/ofd.js` 源码链路预览；压缩包用 `libarchive.js` Worker 按需解压内部文件；邮件用 `postal-mime` / `@kenjiuno/msgreader` 解析正文和附件；OLB/DRA 用 `cfb` 做结构预览；CAD 支持 DXF，DWG 会尽量识别 DXF 误命名和提取内嵌预览图；3D 模型基于 Three.js 支持常见浏览器可渲染格式；Excalidraw 和 draw.io 使用官方预览能力；EPUB 使用 `epubjs`；UMD 电子书按文件结构解析元数据、目录和 zlib 正文段；音频使用浏览器原生播放器；代码/日志使用 `highlight.js` 轻量高亮，HTML 按源码展示。内置 CAD、3D、绘图、压缩包、邮件、EDA、UMD、音频和 EPUB 回归样例来源清楚，方便复现实文件兼容问题。
- **适合平台复用。** 多个系统可以共用同一套私有化 viewer 静态产物，只要升级静态资源即可统一更新预览能力。
- **Vue2 / Vue3 / React / 纯 JS 同步维护。** Vue3 包是 `@flyfish-group/file-viewer3@1.0.11`，Vue2 包是 `@flyfish-group/file-viewer@1.0.11`，React 包是 `@flyfish-group/file-viewer-react@1.0.11`，纯 JS 包是 `@flyfish-group/file-viewer-web@1.0.11`。React 和纯 JS 包不复制渲染器，只通过 iframe 加载 Vue3 基线产物。
...
| EDA | OLB / DRA 使用 CFB 容器解析和可读字符串索引，适合结构初筛 |
