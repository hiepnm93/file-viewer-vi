...
<div class="doc-grid">
  <div class="doc-card">
    <h3>多格式预览</h3>
    <p>内置 194 个扩展名映射，覆盖 Word、Excel、PowerPoint、PDF、OFD、Typst、压缩包、邮件、OLB/DRA、CAD、地理数据、3D 模型、Excalidraw、draw.io、EPUB、UMD、Markdown、图片、音视频、字体、设计资产、结构化数据以及代码/文本。</p>
  </div>
  <div class="doc-card">
    <h3>纯前端渲染</h3>
...
- **Vue2 / Vue3 / React / 纯 JS 同步维护。** Vue3 包是 `@flyfish-group/file-viewer3@1.0.26`，Vue2 包是 `@flyfish-group/file-viewer@1.0.26`，React 包是 `@flyfish-group/file-viewer-react@1.0.26`，纯 JS 包是 `@flyfish-group/file-viewer-web@1.0.26`。Vue3 和 Vue2 安装器都会自动带入样式；React 和纯 JS 包不复制渲染器，只通过 iframe 加载 Vue3 基线产物。
...
公开 GitHub / Gitee 成品仓库用于分发可直接下载使用的构建产物和示例，不包含源码目录。需要源码、二开包或商业自助开通的用户，可以前往 [https://dev.flyfish.group/shop](https://dev.flyfish.group/shop)，请我们喝杯柠檬水后按页面提示自助开通。
