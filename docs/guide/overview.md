...
<div class="doc-grid">
  <div class="doc-card">
    <h3>多格式预览</h3>
    <p>内置 198 个扩展名映射和 24 条预览链路，覆盖 Word、Excel、PowerPoint、PDF、OFD、Typst、XMind 脑图、压缩包、邮件、OLB/DRA/GDS/OASIS、CAD、地理数据、3D 模型、Excalidraw、draw.io、EPUB、UMD、Markdown、图片、音视频、字体、设计资产、结构化数据以及代码/文本。</p>
  </div>
  <div class="doc-card">
    <h3>纯前端渲染</h3>
...
- **格式策略务实。** PPTX 已切换到独立开源的 `@file-viewer/pptx` 原生引擎，重点增强组合图形、主题背景、图片裁剪和 EMF 矢量图片；OFD 基于 `DLTech21/ofd.js` 源码链路预览；Typst 直接读取 `.typ` / `.typst` 源文件，命中格式时才加载浏览器 WASM 编译与 SVG 渲染链路；XMind 使用 `@file-viewer/renderer-mindmap` + `@ljheee/xmind-parser` 解析 XMind 8 / 2020+ 文件包并离线渲染多 sheet 脑图，支持 Pointer / 鼠标 / 触摸拖拽平移和适配画布；压缩包用 `libarchive.js` Worker 按需解压内部文件；邮件用 `postal-mime` / `@kenjiuno/msgreader` 解析正文和附件，并兼容 MBOX 归档；OLB/DRA 做结构树、对象候选、属性、可读字符串和诊断预览，GDSII 读取标准记录并生成 SVG 版图预览，OASIS 先做安全结构索引；CAD 使用 `@flyfish-dev/cad-viewer` 支持 DWG / DXF / DWF / DWFx / XPS，DWG 通过 Worker + LibreDWG WASM 按需解析，DWF/DWFx/XPS 通过 native `dwf-viewer` 渲染 W2D/W3D/XPS 图形；地理数据通过独立 `@file-viewer/renderer-geo` 标准化 GeoJSON/KML/GPX/SHP 并做离线 SVG 预览；3D 模型基于 Three.js 支持常见浏览器可渲染格式，STEP/IFC 等重型工程格式保留 OpenCascade / web-ifc 独立内核路线；Excalidraw 使用官方导出 SVG，draw.io 默认使用随 viewer assets 分发的官方 diagrams.net 离线 viewer 并在异常时回退内置 SVG；EPUB 使用 `epubjs`；UMD 电子书按文件结构解析元数据、目录和 zlib 正文段；音视频使用原生媒体能力并按需补 HLS/MIDI 解析；代码/日志由 `@file-viewer/renderer-text` 使用 `highlight.js` 轻量高亮，HTML 按源码展示；字体、PSD、SQLite、WASM、Parquet、Avro 和 WebArchive 以安全摘要或结构预览为主。内置 CAD、3D、绘图、脑图、压缩包、邮件、EDA、Typst、UMD、音视频、地理数据和结构化数据回归样例来源清楚，方便复现实文件兼容问题。
...
| 能力 | 说明 |
| --- | --- |
| Word | `docx` 使用自研 `@file-viewer/docx`，支持 Worker 解析、连续流式阅读、目录字段缓存和异步分批渲染；`.doc` 使用 `msdoc-viewer`，保留文档页面感 |
...
| 绘图 | Excalidraw 使用官方 `restore` + `exportToSvg`，draw.io 默认使用官方 diagrams.net 离线 `GraphViewer`，异常时回退内置 SVG |
| 电子书 | EPUB 使用 `epubjs`，UMD 解析移动电子书封装、目录和压缩正文 |
| 代码/文本 | `@file-viewer/renderer-text` 使用 `highlight.js` 高亮多语言源码、日志、配置、Notebook、HTTP、Graphviz、Proto、HCL、TeX 和 diff |
