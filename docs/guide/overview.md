...
<div class="doc-grid">
  ...
  <div class="doc-card">
    <h3>多接入路线</h3>
    <p>Vue3 和 Vue2.7 都有对应 npm 包；Vue3 安装器会自动带上样式，Vue2.7 仍可显式引入 CSS。React、纯 JS 和跨技术栈场景通过 iframe 复用 Vue3 基线 viewer 静态产物。</p>
  </div>
</div>
...
- **Vue2 / Vue3 / React / 纯 JS 同步维护。** Vue3 包是 `@flyfish-group/file-viewer3@1.0.12`，Vue2 包是 `@flyfish-group/file-viewer@1.0.12`，React 包是 `@flyfish-group/file-viewer-react@1.0.12`，纯 JS 包是 `@flyfish-group/file-viewer-web@1.0.12`。Vue3 安装器会自动带入样式，Vue2 包仍可显式引入 `dist/style.css`；React 和纯 JS 包不复制渲染器，只通过 iframe 加载 Vue3 基线产物。
