...
<div class="doc-grid">
  <div class="doc-card">
    <h3>多格式预览</h3>
    <p>内置 152 个扩展名映射，覆盖 Word、Excel、PowerPoint、PDF、OFD、Typst、压缩包、邮件、OLB/DRA、CAD、3D 模型、Excalidraw、draw.io、EPUB、UMD、Markdown、图片、音频、视频以及代码/文本。</p>
  </div>
  <div class="doc-card">
    <h3>纯前端渲染</h3>
  ...
  <div class="doc-card">
    <h3>按需异步加载</h3>
    <p>PDF、OFD、Typst、压缩包、邮件、OLB/DRA、CAD、3D 模型、绘图、Office、EPUB、UMD、Markdown 和代码高亮都按需进入对应预览链路，减少无关格式的加载成本。</p>
  </div>
  <div class="doc-card">
    <h3>多接入路线</h3>
...
- **格式策略务实。** PPTX 重点增强组合图形、主题背景、图片裁剪和 EMF 矢量图片；OFD 基于 `DLTech21/ofd.js` 源码链路预览；Typst 直接读取 `.typ` / `.typst` 源文件，命中格式时才加载浏览器 WASM 编译与 SVG 渲染链路；压缩包用 `libarchive.js` Worker 按需解压内部文件；邮件用 `postal-mime` / `@kenjiuno/msgreader` 解析正文和附件；OLB/DRA 用 `cfb` 做结构树、对象候选、属性和诊断预览；CAD 使用 `@flyfish-dev/cad-viewer` 支持 DWG / DXF / DWF / DWFx / XPS，DWG 通过 Worker + LibreDWG WASM 按需解析，DWF/DWFx/XPS 通过 native `dwf-viewer` 渲染 W2D/W3D/XPS 图形；3D 模型基于 Three.js 支持常见浏览器可渲染格式；Excalidraw 和 draw.io 使用官方预览能力；EPUB 使用 `epubjs`；UMD 电子书按文件结构解析元数据、目录和 zlib 正文段；音频使用浏览器原生播放器；代码/日志使用 `highlight.js` 轻量高亮，HTML 按源码展示。内置 CAD、3D、绘图、压缩包、邮件、EDA、Typst、UMD、音频和 EPUB 回归样例来源清楚，方便复现实文件兼容问题。
- **适合平台复用。** 多个系统可以共用同一套私有化 viewer 静态产物，只要升级静态资源即可统一更新预览能力。
- **Vue2 / Vue3 / React / 纯 JS 同步维护。** Vue3 包是 `@flyfish-group/file-viewer3@1.0.23`，Vue2 包是 `@flyfish-group/file-viewer@1.0.23`，React 包是 `@flyfish-group/file-viewer-react@1.0.23`，纯 JS 包是 `@flyfish-group/file-viewer-web@1.0.23`。Vue3 和 Vue2 安装器都会自动带入样式；React 和纯 JS 包不复制渲染器，只通过 iframe 加载 Vue3 基线产物。
...
| 能力 | 说明 |
| --- | --- |
| Word | `docx` 使用 `docx-preview` + Web Worker，`.doc` 使用 `msdoc-viewer`，保留文档页面感 |
...
| 操作栏 | 支持下载原文件、打印完整渲染结果、导出渲染后 HTML，以及文字或图片水印；打印按钮会按当前格式和渲染链路动态显隐 |
...
公开 GitHub / Gitee 成品仓库用于分发可直接下载使用的构建产物和示例，不包含源码目录。需要源码、二开包或商业自助开通的用户，可以前往 [https://dev.flyfish.group/shop](https://dev.flyfish.group/shop)，付费 4.99 后自助开通。
