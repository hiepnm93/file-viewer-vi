...
<div class="doc-grid">
  <div class="doc-card">
    <h3>多格式预览</h3>
    <p>内置 59 个扩展名映射，覆盖 Word、Excel、PowerPoint、PDF、OFD、CAD、Markdown、图片、视频以及代码/文本。</p>
  </div>
  <div class="doc-card">
    <h3>纯前端渲染</h3>
  ...
  <div class="doc-card">
    <h3>按需异步加载</h3>
    <p>PDF、OFD、CAD、Office、Markdown 和代码高亮都按需进入对应预览链路，减少无关格式的加载成本。</p>
  </div>
  <div class="doc-card">
    <h3>双接入路线</h3>
...
- **格式策略务实。** OFD 基于 `DLTech21/ofd.js` 源码链路预览；CAD 支持 DXF，DWG 作为转换提示入口；代码/日志使用 `highlight.js` 轻量高亮，HTML 按源码展示。
- **适合平台复用。** 多个系统可以共用同一套预览站点，只要升级静态资源即可统一更新预览能力。
- **Vue2 / Vue3 同步维护。** Vue3 包是 `@flyfish-group/file-viewer3@1.0.6`，Vue2 包是 `@flyfish-group/file-viewer@1.0.6`，两条包线保持一致的格式能力和 Demo 体验。
- **二开路径明确。** 公开仓库提供成品下载，源码和商业二开通过 [dev.flyfish.group/shop](https://dev.flyfish.group/shop) 自助开通，避免交付渠道混乱。
...
| 图片/视频 | 图片使用浏览器原生能力和轻量查看器，MP4 使用原生视频播放 |
