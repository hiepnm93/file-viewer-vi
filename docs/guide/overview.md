...
- **Vue2 / Vue3 / React / 纯 JS 同步维护。** Vue3 包是 `@flyfish-group/file-viewer3@1.0.13`，Vue2 包是 `@flyfish-group/file-viewer@1.0.13`，React 包是 `@flyfish-group/file-viewer-react@1.0.13`，纯 JS 包是 `@flyfish-group/file-viewer-web@1.0.13`。Vue3 和 Vue2 安装器都会自动带入样式；React 和纯 JS 包不复制渲染器，只通过 iframe 加载 Vue3 基线产物。
