...
这套 API 在多个 npm 包中保持一致: Vue3 使用 `@flyfish-group/file-viewer3@1.0.10`，Vue2.7 使用 `@flyfish-group/file-viewer@1.0.10`，React 使用 `@flyfish-group/file-viewer-react@1.0.10`，纯 JS 使用 `@flyfish-group/file-viewer-web@1.0.10`。React 和纯 JS 包只负责 iframe、参数和二进制推送，默认加载私有化静态目录 `/file-viewer/index.html`。
...
- OFD、CAD、3D 模型、绘图、EPUB、UMD、PDF、Office、Markdown、音频和代码高亮等渲染器均按需异步加载，只有命中文件类型时才拉取对应代码块
