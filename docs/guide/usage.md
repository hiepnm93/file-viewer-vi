...
| `builtinRenderers` | 内置渲染器集合，支持 `all`、`lite`、`none`。当前默认 `all` 以保持历史完整能力；`lite` 只注册图片、音视频和 UMD 等 core 原生低成本链路；代码、文本和 Markdown 请通过 `@file-viewer/renderer-text` 或 `@file-viewer/preset-all` 装配；`none` 适合只通过 `renderers` / renderer preset 显式装配业务需要的格式 |
