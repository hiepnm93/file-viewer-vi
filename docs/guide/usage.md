...
| `builtinRenderers` | 内置渲染器集合，支持 `all`、`lite`、`none`。当前默认 `all` 以保持历史完整能力；`lite` 只注册图片和 UMD 等 core 原生低成本链路；音视频请通过 `@file-viewer/renderer-media` 装配，代码、文本和 Markdown 请通过 `@file-viewer/renderer-text` 装配，也可以直接使用 `@file-viewer/preset-all`；`none` 适合只通过 `renderers` / renderer preset 显式装配业务需要的格式 |
