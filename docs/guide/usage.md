...
这套 API 在多个 npm 包中保持一致: Vue3 使用 `@flyfish-group/file-viewer3@1.0.26`，Vue2.7 使用 `@flyfish-group/file-viewer@1.0.26`，React 使用 `@flyfish-group/file-viewer-react@1.0.26`，纯 JS 使用 `@flyfish-group/file-viewer-web@1.0.26`。React 和纯 JS 包只负责 iframe、参数和二进制推送，默认加载私有化静态目录 `/file-viewer/index.html`。
...
`.docx`、`.docm`、`.dotx`、`.dotm` 使用 `docx-preview` 做高保真页面渲染。当前默认会在 Web Worker 内完成 ZIP/XML 解析和 HTML 构建，主线程只把同一份 docx-preview HTML 按页面分批挂载，再执行宽度自适应、打印和导出适配，减少复杂 Word 文档打开时的首屏卡顿。
...
浏览器 Worker 不能直接操作真实页面 DOM，因此这里采用“Worker 构建 docx-preview HTML + 主线程分批挂载”的标准链路，而不是自研简化版 Word 渲染器。少量结构复杂的 Word 文件如果触发 Worker DOM 兼容边界，预览器会按 `workerTimeout` 超时回退到同一套 `docx-preview` 原生主线程渲染，避免永久停留在 loading。极端超大或结构异常的 Word 文件仍建议业务侧提供原文件下载，或在服务端转为 PDF/OFD 后再做稳定版式预览。
...
字体文件 `ttf/otf/woff/woff2` 会用 FontFace 临时注册到预览容器并展示样张。`psd` 会按需加载 `ag-psd` 展示尺寸、图层和预览图；`ai` 如果是 PDF-backed 文件会进入 PDF 预览，否则展示安全摘要；`eps` 不执行 PostScript，仅展示文本摘要。`sqlite`、`parquet`、`avro`、`wasm` 和 `webarchive` 都走结构预览链路，目标是快速判断文件内容，不替代数据库客户端或专业分析工具。
