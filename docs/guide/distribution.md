...
| npm 标准生态 | [生态组件总览](/guide/ecosystem) | `@file-viewer/core`、`@file-viewer/pptx`、Vue3、Vue2.7、Vue2.6、React、React Legacy、Pure Web、jQuery、Svelte 全线标准包 |
...
| Pure Web / script 标签 | `@file-viewer/web` | `@flyfish-group/file-viewer-web` |
...
Core、PPTX、Vue3、Vue2.7、Vue2.6、React、React Legacy、Pure Web、jQuery、Svelte 和历史兼容 tarball 都会随开源总仓库一起生成。`file-viewer3` 非 scoped 兼容包仍会同步发布到 npm，但它和 `@flyfish-group/file-viewer3` 包体重复，开源总仓库下载区只保留 `flyfish-group-file-viewer3-*.tgz` 这一份 Vue3 兼容 tarball。React tarball 依赖 web viewer 包，离线安装时请按 npm 依赖关系一起放入本地源或依次安装。
