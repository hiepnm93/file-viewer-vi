...
```bash
npm install --save @flyfish-group/file-viewer-react@1.0.19
```
...
`@flyfish-group/file-viewer-react` 依赖 `@flyfish-group/file-viewer-web@^1.0.19`。使用 `npm install` 或已允许 pnpm 安装脚本后，web 包会把随包携带的 Vue3 基线 viewer 产物复制到宿主项目的 `public/file-viewer`，所以默认地址就是:
