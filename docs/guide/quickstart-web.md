...
本地验证可以执行下面的命令。它会先校验 adapter demo 的静态产物，再用真实浏览器打开 demo 首页、`/vue3.html`、`/jquery.html`、`/svelte-action.html` 和 `/manual-iife.html`，验证 React wrapper、Vue 3 wrapper、Pure Web wrapper、jQuery wrapper、Svelte action 与普通 script 标签全局包都能正确创建 viewer iframe。
