...
| `createViewerControllerHandle()` | 创建可复用的 controller handle，适合框架 wrapper 封装 |

`options` 与 Vue、React、jQuery、Svelte wrapper 保持一致，支持主题、水印、搜索、统一缩放、下载、打印、导出 HTML、beforeOperation 前置校验、生命周期 hooks、压缩包缓存和格式专项参数。
