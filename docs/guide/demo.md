...
| 文档比对页 | `/compare.html` | 左右并排预览两份文档，支持示例、URL、本地上传、交换、重置、同步滚动、搜索高亮和行级定位 |
...
比对页顶部提供“同步滚动”“隐藏 PDF 工具栏”“交换”“重置”，中间搜索区提供关键词搜索、上一个、下一个和行级定位。同步滚动按两侧真实滚动容器的滚动比例联动，不要求两份文档页数完全一致；搜索会在两侧同时高亮命中并滚动到当前结果；行级定位会复用预览器抽取出的通用锚点，文本类文档通常定位到段落/行块，PDF 会优先使用可用文本层和页面锚点。
...
这项功能做的是视觉并排预览，不是语义 diff。它不会自动标红文本改动，也不会解析 Office 修订痕迹；如果需要合同逐字差异、表格单元格差异或 PDF OCR 差异，请在业务侧接入专门的 diff 服务，再把结果作为文件或 HTML 交给预览器展示。预览器已经提供 `collectDocumentAnchors()`、`searchDocument()` 和 `getDocumentTextChunks()`，业务侧可以基于这些通用结构继续实现 AI 溯源、向量化召回、命中高亮和审计记录。
