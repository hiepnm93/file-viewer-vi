...
const normalizeGeoJson = (value, t) => {
    ...
    if ((candidate === null || candidate === void 0 ? void 0 : candidate.type) && geometryTypes.has(candidate.type)) {
        return {
            type: 'FeatureCollection',
...
const normalizeShapefileOutput = (value, t) => {
...
const isWgs84Projection = (projection) => {
    return ['EPSG:4326', 'CRS:84', 'WGS84', 'WGS 84'].includes(projection.toUpperCase());
};
const normalizeProjectionAlias = (value) => {
    const raw = value === null || value === void 0 ? void 0 : value.trim();
    if (!raw) {
        return undefined;
    }
...
const inferProjectionFromBounds = (bounds, options) => {
    if (!bounds || (options === null || options === void 0 ? void 0 : options.inferProjection) === false) {
        return undefined;
    }
...
const registerCommonProj4Defs = (proj4) => {
...
const createProjectionTransformer = async (projection, t) => {
...
const transformGeometry = (geometry, transform) => {
    ...
    switch (geometry.type) {
        ...
        case 'GeometryCollection':
            return { type: 'GeometryCollection', geometries: geometry.geometries.map(item => transformGeometry(item, transform)) };
    }
};
const validatePositionBounds = (bounds, t) => {
    if (!bounds) {
        return;
    }
...
const resolveSourceProjection = (collection, declaredProjection, options) => {
    const rawBounds = collectBounds(collection.features);
    const projection = normalizeProjectionAlias(options === null || options === void 0 ? void 0 : options.projection) ||
        normalizeProjectionAlias(declaredProjection) ||
        inferProjectionFromBounds(rawBounds, options) ||
        'EPSG:4326';
    return projection;
};
export const parseFileViewerGeoData = async (buffer, type, options, t) => {
...
const buildSvgPreview = (parsed, t) => {
...
const fitMapToBounds = (map, bounds, padding) => {
    ...
    map.fitBounds([[bounds.minX, bounds.minY], [bounds.maxX, bounds.maxY]], { padding, duration: 0, maxZoom: 16 });
};
const createZoomState = (map, bounds) => {
...
const mountMapLibre = async (host, root, parsed, options, basemap, t) => {
    ...
    const resizeObserver = ResizeObserverCtor
        ...
        : null;
    resizeObserver === null || resizeObserver === void 0 ? void 0 : resizeObserver.observe(host);
    const fitButton = root.querySelector('[data-geo-action="fit"]');
    fitButton === null || fitButton === void 0 ? void 0 : fitButton.addEventListener('click', fit);
    fitButton === null || fitButton === void 0 ? void 0 : fitButton.setAttribute('title', t('geo.action.fit'));
    return {
        map,
        cleanup() {
...
export default async function renderGeo(buffer, target, type, context) {
    ...
    const options = (_a = context === null || context === void 0 ? void 0 : context.options) === null || _a === void 0 ? void 0 : _a.geo;
    ...
    try {
        ...
        if ((options === null || options === void 0 ? void 0 : options.preferMapEngine) !== false) {
            try {
