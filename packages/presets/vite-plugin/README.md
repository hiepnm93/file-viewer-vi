...
需要更多格式时安装对应 renderer 包，例如 `@file-viewer/renderer-ofd`、`@file-viewer/renderer-presentation`、`@file-viewer/renderer-cad`、`@file-viewer/renderer-typst`、`@file-viewer/renderer-archive`。
...
当前插件会为已经拆出的 renderer 包生成导入：PDF、OFD、Presentation、CAD、Typst、压缩包、邮件、EPUB、代码/Markdown、图片、媒体、XMind 和 Geo。尚未拆出的 Word、Excel、绘图、3D、EDA 和数据资产格式会给出明确提示；在拆包完成前，全量体验仍可直接使用 `@file-viewer/preset-all`。
