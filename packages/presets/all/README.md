...
- 你希望先用一个 preset 接入，后续再替换为 `renderer-text`、`renderer-image`、`renderer-word`、`renderer-pdf`、`renderer-cad` 等更细的组合。
