...
export const fileViewerAllRendererPlugin: FileViewerRendererPlugin<BrowserRendererHandler> = {
  ...
  definitions: DEFAULT_RENDERER_DEFINITIONS.filter(definition => !['cad', 'mindmap', 'pdf'].includes(definition.id)),
  handlers: allRendererHandlers.filter(handler => !['cad', 'mindmap', 'pdf'].includes(handler.rendererId)),
};
...
export const allRenderers: FileViewerRendererPreset<BrowserRendererHandler> = {
  ...
  renderers: [pdfRenderer, cadRenderer, mindmapRenderer, fileViewerAllRendererPlugin],
};
