...
Current framework-neutral browser renderers still inside core include media, images, UMD, Excel/Spreadsheet, PDF, OFD, email, EDA, CAD, Typst, EPUB, 3D models, drawing files, data assets, and archives. Code, text, and Markdown rendering has moved to `@file-viewer/renderer-text`; Word DOCX/DOC/RTF/ODT rendering has moved to `@file-viewer/renderer-word`, PowerPoint rendering has moved to `@file-viewer/renderer-presentation`, XMind rendering has moved to `@file-viewer/renderer-mindmap`, and geospatial rendering has moved to `@file-viewer/renderer-geo`. Core-only installs now avoid direct Word, PPTX, XMind, Geo, `highlight.js`, and `marked` engine dependencies.
