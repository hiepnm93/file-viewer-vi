...
标准 Vue 2.6 native wrapper 包，提供 `Vue.use()` 插件安装和局部组件两种方式。它不依赖 Vue 2.7 Composition API，组件内部通过本包本地 controller 调用 `@file-viewer/core` 与 core browser engine 挂载完整预览器。
