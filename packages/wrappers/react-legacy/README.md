...
标准 React legacy native wrapper 包，面向 React 16.8 和 React 17。它不依赖 React 18/19 的能力，也不依赖新的 JSX transform，内部通过本包本地 controller 调用 `@file-viewer/core` 与 core browser engine 挂载完整预览器。
