...
标准 Svelte native wrapper 包，提供 Svelte 组件和 action 两种接入方式。组件和 action 都通过本包本地 controller 调用 `@file-viewer/core` 与 core browser engine 挂载完整预览器；SSR 环境下只在浏览器端挂载。
