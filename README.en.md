...
The current version follows the npm registry `latest` dist-tag. The ecosystem publishes 26 npm targets: 21 standard component / core / renderer / preset packages and 5 historical aliases. New integrations should prefer the `@file-viewer/*` standard package names; existing applications using `@flyfish-group/*` or `file-viewer3` continue to receive the same versioned capability set.
