...
| `builtinRenderers`             | Built-in renderer set: `all`, `lite`, or `none`. The current default is `all` for historical compatibility. Use `lite` for core-native image, audio/video, and UMD only; use `none` when renderer packages or presets should be the only installed rendering surface. Code, text, and Markdown preview should be provided by `@file-viewer/renderer-text` or `@file-viewer/preset-all`. |
