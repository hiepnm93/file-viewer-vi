...
The current version follows the npm registry `latest` dist-tag. The ecosystem publishes 32 npm targets: 27 standard component / core / renderer / preset / tooling packages and 5 historical aliases. New integrations should prefer the `@file-viewer/*` standard package names; existing applications using `@flyfish-group/*` or `file-viewer3` continue to receive the same versioned capability set.
