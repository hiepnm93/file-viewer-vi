...
| XMind mind maps                | `xmind`                                                                                                                                                                                                                                                                                                                        | `@ljheee/xmind-parser` parses XMind 8 XML and XMind 2020+ JSON packages; the browser renderer shows sheets, nodes, labels, notes, links, markers, images, outline, Pointer / mouse / touch drag-to-pan canvas, mobile pinch zoom, keyboard panning, unified toolbar state sync, embedded WebView `buttons` compatibility, fit-to-view, zoom, search, print, and HTML export | Mind maps, planning boards, knowledge structures, meeting notes |
...
The current version follows the npm registry `latest` dist-tag. The ecosystem publishes 35 npm targets: 30 standard component / core / renderer / preset / tooling packages and 5 historical aliases. New integrations should prefer the `@file-viewer/*` standard package names; existing applications using `@flyfish-group/*` or `file-viewer3` continue to receive the same versioned capability set.
...
| Full renderer preset                       | [`@file-viewer/preset-all`](https://www.npmjs.com/package/@file-viewer/preset-all)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           | none                                                                                                                                                     | `latest`       | One-shot composition for Word, PDF, OFD, PPTX, CAD, Draw.io/Excalidraw, Typst, XMind, archives, email, ebooks, code/Markdown, images, audio/video, and the remaining complete core format matrix                            |
...
| `builtinRenderers`             | Built-in renderer set: `all`, `lite`, or `none`. The current default is `all` for historical compatibility. Use `lite` for core-native image and UMD only; use `none` when renderer packages or presets should be the only installed rendering surface. Audio/video should be provided by `@file-viewer/renderer-media`; code, text, and Markdown preview should be provided by `@file-viewer/renderer-text`; `@file-viewer/preset-all` aggregates both. |
